...
router = APIRouter(prefix="/lab8", tags=["protected"])
...
async def get_lab_page(request: Request, token: str = Depends(get_token_from_cookie)):
    ...
    variant = await app_settings.database.load_linal_lab8_variant(student_id=current_student.id)
    ...
    return templates.TemplateResponse("linear_algebra/lab8.html", context=web_context)


@router.post("/check", tags=["checker"])
async def check_lab(request: Request, user_answer: LinalLab8Response, token: str = Depends(get_token_from_cookie)) -> dict[str, Any]:
    username = get_username_by_jwt(token)
    ...
    is_correct_answer = linear_algebra.lab8.check_lab(condition=variant, user_answer=user_answer)

    marks = current_student.marks
    if is_correct_answer and not marks[7].result:  # TODO: if two requests from different labs arrive at the same time, the score may get lost
        marks[7].result = True
        marks[7].approve_date = str(datetime.datetime.now().date())
